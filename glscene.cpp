...
namespace {
	VertexAndFaceArraysPtr vaoFromOBJPath(const std::string& path, const ShaderProgram& program) {
		...
		if (!mesh.face_texcoords.empty()) {
			...
			// Your code goes here.
			// 1 Flatten.
			auto flat_tangents = flatten_attribute(mesh.face_tangents, mesh.tangents);
			auto flat_bitangents = flatten_attribute(mesh.face_tangents, mesh.bitangents);
			// 2 Upload to "vTangent" and "vBitangent".
			vao->uploadAttribute(flat_tangents, program.getAttribLocation("vTangent"));
			vao->uploadAttribute(flat_bitangents, program.getAttribLocation("vBitangent"));
		}
...
namespace graphics101 {
	...
	void GLScene::mouseMoveEvent(QMouseEvent* event) {
        ...
        diff *= M_PI / glm::min(width_and_height[0], width_and_height[1]);
